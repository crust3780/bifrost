...
function VKStatusBadge({ vk }: { vk: VirtualKey }) {
	...
	const isExpired = !!vk.expires_at && new Date() > new Date(vk.expires_at);
	const variant = !vk.is_active ? "secondary" : isExpired || isExhausted ? "destructive" : "default";
	const label = !vk.is_active ? "Inactive" : isExpired ? "Expired" : isExhausted ? "Exhausted" : "Active";
